...
class AVL
{
   private:
      ...
      void makeDeletion(Node<T>*& nodeAbove, Node<T>*& node, Node<T>*& nodetoAttach);
 ...
 *  Purpose:  A private function that return the height factor
 *            of a node in AVL
 **************************************************************/
template <typename T>
...
void AVL<T>::makeDeletion(Node<T> *&node)
{
   ...
   {
      makeDeletion(node, node->mRight, node->mLeft);
      node = node->mRight;   
   }   
...
template <typename T>
void AVL<T>::makeDeletion(Node<T>*& oneAbove, Node<T>*& attachPoint,
                          Node<T>*& nodetoAttach)
{
   // Locate the smallest node in the right subtree
   // by moving as far to the left as possible.
   if (attachPoint->mLeft != NULL)
   {
      makeDeletion(attachPoint, attachPoint->mLeft, nodetoAttach);

      if (getBalance(attachPoint) < -1)
      {
         if (getBalance(attachPoint->mLeft) == -1)
            rotateRight(attachPoint);
         else
            rotateLeftRight(attachPoint);
      }

      else if (getBalance(attachPoint) > 1)
      {
         if (getBalance(attachPoint->mRight) == 1)
            rotateLeft(attachPoint);
         else
            rotateLeftRight(attachPoint);
      }
   }
   else
   {
      ...
      attachPoint->mLeft = nodetoAttach;
...
int AVL<T>::maxHeight(Node<T> *node1, Node<T> *node2)
{
   ...
   heightNode1 = getHeight(node1);
   heightNode2 = getHeight(node2);
...
void AVL<T>::rotateLeft(Node<T> *&node)
{
   ...
   tmp->mLeft = node;
   node->mHeight = maxHeight(node->mLeft, node->mRight) + 1;
   tmp->mHeight = maxHeight(tmp->mRight, node) + 1;
   node = tmp;
}
...
void AVL<T>::rotateRight( Node<T> *&node)
{
   ...
   tmp->mRight = node;
   node->mHeight = maxHeight(node->mLeft, node->mRight) + 1;
   tmp->mHeight = maxHeight(tmp->mLeft, node) + 1;
   node = tmp;
}
